...
public class ODLMapleApp extends MapleAppBase{
	...
	private static final String[] H12_HIGH_PATH = {
			H1,
			"openflow:1:3",
			"openflow:2:2",
			"openflow:4:1"
	};
	private static final String[] H12_LOW_PATH = {
			H1,
			"openflow:1:4",
			"openflow:3:2",
			"openflow:4:1"
	};
	private static final String[] H21_HIGH_PATH = {
			H2,
			"openflow:4:4",
			"openflow:2:1",
			"openflow:1:1"
	};
	private static final String[] H21_LOW_PATH = {
			H2,
			"openflow:4:5",
			"openflow:3:1",
			"openflow:1:1"
	...
	private void checkStaticH12(MaplePacket pkt) {
		if (pkt.IPv4SrcIs(IPv4.toIPv4Address(H1)) 
				&& pkt.IPv4DstIs(IPv4.toIPv4Address(H2))) {
			String[] path = null;
			...
			pkt.setRoute(path);
		} else if (pkt.IPv4SrcIs(IPv4.toIPv4Address(H2)) 
				&& pkt.IPv4DstIs(IPv4.toIPv4Address(H1))) {
			String[] path = null;
